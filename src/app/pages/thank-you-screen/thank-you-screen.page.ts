...
export class ThankYouScreenPage implements OnInit {
  ...
  subtitle =
    "We believe the winners will be the ones who re-wire their enterprise to out-think, out-do and outmaneuver disruption. They will be the ones who endure – by sustaining the value of innovation – delivering value across their entire enterprise.";
  formTag: string;
  ...
  ngOnInit() {
    ...
    this.route.queryParams.subscribe(params => {
      if (params.tag !== undefined) {
          ...
          this.apiCallService.getFormFromTag(this.formTag).subscribe(res => {
            ...
            if (this.currentFormAnswers) {
              ...
              this.loading.onDidDismiss().then(() => {
                if (!this.currentFormAnswers) {
                  const message =
                    "Impossible to retrieve corresponding form, please try to refresh the page or verify your connection...";
        ...
        const message =
          "The element you are requesting doesn't exist, please try again";
        this.presentToast(message);
      }
    });
  }
  ...
  async formatData(height: number) {
    if (this.currentFormAnswers && this.currentFormAnswers.formattedRes) {
    ...
    } else if (this.currentFormAnswers) {
      // We need to process the data to have them in a readable and usable way
      ...
      this.questions.forEach(value => {
        if (
          !value.includes("Tag") &&
          !value.includes("company are you from") &&
          !value.includes("your job title") &&
          !value.includes("country is your team") &&
          !value.includes("industry sector are you") &&
          !value.includes("biggest business problem") &&
          !value.includes(
            "By completing this assessment, you agree with Capgemini's privacy policy"
          ) &&
          value.length > 0
        ) {
          ...
          if (i % 5 === 0) {
            ...
            this.maturity_score[qcount] =
              (this.maturity_score[qcount] / 4) * 10;
            const labelParam = new ScoreData(
              this.labels[qcount - 1],
              this.maturity_score[qcount],
              this.priority_score[qcount]
            );
  ...
  async updateDbWithFormattedData() {
    this.apiCallService.updateFormFromTag(
      this.currentFormAnswers.id,
      this.organization,
      this.results,
      this.country,
      this.industry,
        ...
        tooltips: {
          callbacks: {
            ...
            label: (item, data) => {
              ...
              return (
                datasetLabel + ": " + "(" + dataPointx + ", " + dataPointy + ")"
              );
            }
          }
        },
  ...
  orderResultsByPriority() {
    ...
    ); // Sorting array by lowest priority
  }
  ...
  async initHorizontalPriorityChart() {
    ...
    const labels = this.getResultsLabels();
    const datasets = [
      { data: priorities, backgroundColor: this.priorityColor }
    ];
  ...
  async initHorizontalMaturityChart() {
    this.results.sort((a, b) => (a.maturity > b.maturity ? 1 : -1)); // Sorting array by lowest maturity
    const maturities = this.getResultsMaturities();
    const labels = this.getResultsLabels();

    const datasets = [
      { data: maturities, backgroundColor: this.maturityColor }
    ];
